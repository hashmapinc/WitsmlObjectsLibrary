...
/**
 * <p>Java class for QuantityClassKind.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * <simpleType name="QuantityClassKind">
 ...
 *     <enumeration value="not a measure"/>
 *   </restriction>
 * </simpleType>
 * </pre>
 * 
 */
@XmlType(name = "QuantityClassKind")
@XmlEnum
public enum QuantityClassKind {
    ...
    /**
     * A unitless quantity is a quantity which has no unit of measure symbol, but could be a real physical measurement. Examples would be a count, pH, wire gauge (AWG and BWG) and shoe size.
     * 
     * This is different from a dimensionless quantity which represents a ratio whose units of measure have cancelled each other. DImensionless quantities can have units of measure (like ppm or %) or may not have a displayable unit of measure symbol (in which case the units symbol Euc is used in a data transfer).
     * 
     * Units derived from a unitless number simply ignore the unitless part. For example, the unit for counts per hour is just inverse hours (1/hr).
     * 
     */
    ...
    /**
     * The "not a measure" quantity class represents data values which are not measures at all. This would include strings, ordinal numbers, index values and other things for which the concept of units of measure is irrelevant.
     * 
     */
    @XmlEnumValue("not a measure")
    NOT_A_MEASURE("not a measure");
    private final String value;

    QuantityClassKind(String v) {
        value = v;
    }
    ...
    public static QuantityClassKind fromValue(String v) {
        for (QuantityClassKind c: QuantityClassKind.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
